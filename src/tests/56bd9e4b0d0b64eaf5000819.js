...
describe('Combine three objects', () => {
  it('tests', () => {
    assert.deepEqual(combine(objA, objB, objC), {
      a: 18,
      b: 20,
      c: 36,
      d: 14,
    ...
    assert.deepEqual(combine(objA, objC, objD), {
      a: 15,
      b: 20,
      c: 33,
      d: 11,
...
describe('Handle empty objects', () => {
  it('tests', () => {
    ...
    assert.deepEqual(combine(objA, objC, {}), {
      a: 15,
      b: 20,
      c: 30,
      d: 11,
  ...
  it('tests', () => {
    ...
    const genObject = () =>
      Array.from({ length: genNumber(26) })
        .map(() => [genKey(), genNumber(100)])
        .reduce((o, [k, v]) => ({ ...o, ...{ [k]: v } }), {});

    const genObjects = () =>
      Array.from({ length: genNumber(10) }).map(genObject);

    const doTest = (os) => assert.deepEqual(combine(...os), __combine(...os));
